/* JSModeler 0.41 - http://www.github.com/kovacsv/JSModeler */
...
module.exports = function( THREE ) {
    ...
    */
    JSM.TriangulatePolygon = function (polygon)
    {
        ...
        // 2020-04-01 Chugaynov A. S.
        // Change triangulations algorithm to Earcut (The fastest and smallest JavaScript polygon triangulation library)
        // return Earcut(polygon2D);
    };
    /**
    ...
    */
    JSM.BodyTextureProjection = function ()
    {
        ...
        // 2021-05-04 Chugaynov A. S.
        // Rotate texture coordinate system from default to correct texture bricks on house walls
        // this.SetCubic (new JSM.Coord (0.0, 0.0, 0.0), new JSM.Coord (0.0, 0.0, 1.0), new JSM.Coord (1.0, 0.0, 0.0), new JSM.Coord (0.0, 1.0, 0.0));
    };
    ...
    */
    JSM.CalculatePolygonCubicTextureCoords = function (body, index, normal)
    {
        var result = [];
        var polygon = body.GetPolygon (index);
        body.GetTextureProjection()
        var system = body.GetTextureProjection ().GetCoords ();

        // // 2021-05-04 Chugayno A. S.
        // // Rotate texture without geometry rotation
        // var system = {
        //     e1: new JSM.Coord(0, 1, 0),
        //     e2: new JSM.Coord(0, 0, 1),
        //     e3: new JSM.Coord(1, 0, 0),
        //     origo: new JSM.Coord(0, 0, 0)
        // };
    ...
    */
    JSM.GeneratePolyTorus = function (basePolygon, outerRadius, outerSegmentation, isCurved)
    {
        ...
        var step = (2.0 * Math.PI) / outerSegmentation;
        var j, rotated;
        for (i = 0; i < outerSegmentation; i++) {
            for (j = 0; j < innerSegmentation; j++) {
                rotated = circle[j].Clone ().Rotate (axisDir, i * step, origo);
    ...
    */
    ...
    JSM.Renderer.prototype.SetClearColor = function (red, green, blue, alpha)
    {
        this.context.clearColor (red, green, blue, (typeof alpha === 'undefined') ? 1.0 : alpha);
    };
    ...
    JSM.PointCloudRenderer.prototype.SetClearColor = function (red, green, blue, alpha)
    {
        this.context.clearColor (red, green, blue, (typeof alpha === 'undefined') ? 1.0 : alpha);
    };
    ...
    /**
     * Function: OrderScenePolygons
     * Description: Order body polygons from front to back depending on the eye position.
     * Parameters:
     *	bodies {[[Body1, Materials1], ... [BodyN, MaterialsN]]} array of all scene bodies
    *	eye {Coord} the camera eye position
    *	center {Coord} the camera center position
    * Returns:
    *	[{bodyIndexN, polygonOnBodyIndexNM}, ... {bodyIndexN, polygonOnBodyIndexN(M + 1)}]
    */
    JSM.OrderScenePolygons = function (bodies, eye, center)
    {
        ...
        function GetPolygonCenter (body, p)
        {
        ...
        function CalculatePolygonValues (body)
        {
            ...
            for (i = 0; i < body.PolygonCount (); i++) {
                ...
                polygonCenter = GetPolygonCenter (body, i);
        ...
        function PolygonIsFrontOfPlane (body, s, p)
        {
            var sPlane = polygonPlanes[s];
            var pPlane = polygonPlanes[p];

            // Reget by body
            s = src_body_polygons[s];
            p = src_body_polygons[p];
        ...
        function NeedToChangeOrder (body, s, p)
        {
            ...
            if (PolygonViewOverlap (s, p)) {
                if (PolygonIsFrontOfPlane (body, s, p)) {
        ...
        function ReorderPolygons (body)
        {
            ...
            for (i = 0; i < count - 1; i++) {
                for (j = 0; j < count - i - 1; j++) {
                    if (NeedToChangeOrder (body, ordered[j], ordered[j + 1])) {
                        SwapArrayValues (ordered, j, j + 1);
                    }
                }
            }
        ...
        var ordered = [];
        var src_body_polygons = [],
            src_body_indexes = [];
        var needToChangeOrderCache = [];

        var bodyPolygonsCount = 0, pos = 0;
        for(var i = 0; i < bodies.length; i++){
            // console.log(bodies[i][0].PolygonCount());
            bodyPolygonsCount = bodies[i][0].PolygonCount();

            for(var j = 0; j < bodyPolygonsCount; j++){
                ordered.push (pos);
                needToChangeOrderCache.push ([]);

                src_body_indexes[pos] = i;
                src_body_polygons[pos] = j;

                pos++;

                for(var k = 0; k < bodyPolygonsCount; k++){
                    needToChangeOrderCache[j].push (null);
                }
            }
        }
        ...
        for(i = 0; i < bodies.length; i++){
            CalculatePolygonValues (bodies[i][0]);
            OrderPolygonsByMaxViewDistance ();
        }


        for(i = 0; i < bodies.length; i++){
            ReorderPolygons (bodies[i][0]);
        }
        ...
        for(i = 0; i < ordered.length; i++){
            ...
            result.push([
                src_body_indexes[sIndex],
                src_body_polygons[sIndex]
            ]);
        }
    ...
    //	}
    //
    //	CalculatePolygonValues (body);
    //	OrderPolygonsByMaxViewDistance ();
    //	ReorderPolygons (body);
    //
    //	result = ordered;
    //	return result;
    };
    ...
    */
    ...
    /**
     * Cowork, 2016/07/18 Modification
     *
    ...
    *	drawMode {string} draw mode ('HiddenLinePainter')
    *	needClear {boolean} clear the display before draw
    *	drawer {drawer object} the drawer object
    */
    JSM.DrawProjectedBodies = function (bodies, camera, drawMode, needClear, drawer){
        ...
        function GetProjectedPolygonFromBody (body, polygon)
        {
        ...
        var i, projected, materialIndex, color;
        if (drawMode === 'HiddenLinePainter') {
            // Return array of ordered polygons	[{bodyIndexN, polygonOnBodyIndexNM}, ... {bodyIndexN, polygonOnBodyIndexN(M + 1)}]
            var orderedPolygons = JSM.OrderScenePolygons (bodies, eye, center);

            var bodyIndex = 0, polygon = {}, body = [], bodyMaterials = {}, polygonOnBodyIndex = 0;
            for (i = 0; i < orderedPolygons.length; i++) {
                bodyIndex = orderedPolygons[i][0];
                polygonOnBodyIndex = orderedPolygons[i][1];

                body = bodies[bodyIndex][0];
                bodyMaterials = bodies[bodyIndex][1];

                if (bodyMaterials === undefined || bodyMaterials === null) {
                    bodyMaterials = new JSM.Materials ();
                }

                polygon = body.GetPolygon (polygonOnBodyIndex);

                projected = GetProjectedPolygonFromBody (body, polygon);
                materialIndex = polygon.GetMaterialIndex ();
                color = bodyMaterials.GetMaterial (materialIndex).diffuse;
    ...
    */
    ...
    JSM.Navigation.prototype.changeMouseCombination = function (mouse){
        // this.MOUSE = {
        //     ROTATE: 1,
        //     PAN: 3
        // };
    ...
    JSM.Navigation.prototype.OnTouchMove = function (event)
    {
        ...
        var ratio = 0.0;
        if (this.touch.fingers === this.TOUCH.ROTATE) { // 1
            if (!this.cameraEnableOrbit) {
                return;
            }
            ratio = 0.5;
            this.Orbit (this.touch.diff.x * ratio, this.touch.diff.y * ratio);
        } else if (this.touch.fingers === this.TOUCH.ZOOM) { // 2
            if (!this.cameraEnableZoom) {
                return;
            }
            ratio = 0.005;
            this.Zoom (this.touch.diff.x * ratio);
        } else if (this.touch.fingers === this.TOUCH.PAN) { // 3
            if (!this.cameraEnablePan) {
                return;
            }
            var eyeCenterDistance = this.camera.eye.DistanceTo (this.camera.center);
    ...
    JSM.Navigation.prototype.changeTouchCombination = function (touch){
        // this.TOUCH = {
        //     ROTATE: 1,
        //     PAN: 3,
        //     ZOOM: 2
    ...
    JSM.SoftwareViewer.prototype.Draw = function ()
    {
        ...
        JSM.DrawProjectedBodies (this.bodies, this.camera, this.drawMode, false, this.drawer);
    ...
    JSM.Viewer.prototype.SetClearColor = function (red, green, blue, alpha)
    {
        this.renderer.SetClearColor (red, green, blue, (typeof alpha === 'undefined') ? 1.0 : alpha);
    };
    ...
    JSM.PointCloudViewer.prototype.SetClearColor = function (red, green, blue, alpha)
    {
        this.renderer.SetClearColor (red, green, blue, (typeof alpha === 'undefined') ? 1.0 : alpha);
    };
    ...
    */
    ...
    //			threeGeometry.faces.push (face);
                ...
                // 2021-03-22 Chugainov A. S. (Unpacked Version)
                ////////////////////////////////////////////////////
                // 2019-11-14 Чугайнов А. С.
                // Изменение кода позволило избавиться от излишних вертексов и очень сильно облегчило сцену
                var existingVertexIndex = function(vector){
                    for(var i = 0; i < threeGeometry.vertices.length; i++){
                        if(threeGeometry.vertices[i].equals(vector)){
                            return i;
                        }
                    }

                    // Vertex is not presented
                    return -1;
                };

                var vertexA = new THREE.Vector3(vertex1.x, vertex1.y, vertex1.z),
                    indexVertexA = existingVertexIndex(vertexA);
                if(indexVertexA === -1) {
                    threeGeometry.vertices.push(vertexA);
                    indexVertexA = threeGeometry.vertices.length - 1;
                }


                var vertexB = new THREE.Vector3(vertex2.x, vertex2.y, vertex2.z),
                    indexVertexB = existingVertexIndex(vertexB);
                if(indexVertexB === -1) {
                    threeGeometry.vertices.push(vertexB);
                    indexVertexB = threeGeometry.vertices.length - 1;
                }

                var vertexC = new THREE.Vector3(vertex3.x, vertex3.y, vertex3.z),
                    indexVertexC = existingVertexIndex(vertexC);
                if(indexVertexC === -1) {
                    threeGeometry.vertices.push(vertexC);
                    indexVertexC = threeGeometry.vertices.length - 1;
                }

                var face = new THREE.Face3(indexVertexA, indexVertexB, indexVertexC);
                threeGeometry.faces.push(face);
                ////////////////////////////////////////////////////
    ...
    JSM.ThreeViewer.prototype.SetClearColor = function (color, alpha)
    {
        this.renderer.setClearColor (new THREE.Color (color), (typeof alpha === 'undefined') ? 1.0 : alpha);
        this.DrawIfNeeded ();
    };
