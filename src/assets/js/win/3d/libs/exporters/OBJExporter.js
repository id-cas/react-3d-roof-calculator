module.exports = function( THREE ) {
	...
	THREE.OBJExporter.prototype = {
		...
		parse: function (object, options) {
			...
			var onlyVisible = options.onlyVisible || true;
						...
						for (i = 0, l = vertices.count; i < l; i++, nbVertex++) {
							...
							// if (skinIndices == undefined) {
								...
								if (geometry.morphTargetInfluences !== undefined) {
									...
									let morphLength = geometry.morphAttributes.position.length;
									//console.log("Morph Length " + morphLength);
									for (let mt = 0; mt < morphLength; mt++) {
								...
								for (var k = 0; k < 4; k++) {
									var tempVector = null;
									if (geometry.morphTargetInfluences !== undefined) {
										tempVector = new THREE.Vector4(morphVector.x, morphVector.y, morphVector.z);
									} else {
										tempVector = new THREE.Vector4(vertex.x, vertex.y, vertex.z);
									}
			...
			object.traverse(function (child) {
				...
				if (child instanceof THREE.Mesh) {
					...
					// Source state. Mod 2021-04-22 Chugaynov A.S.
					// parseMesh(child);
